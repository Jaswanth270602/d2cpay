...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $template = DB::table('services')->where('id', 36)->first()
            ?: DB::table('services')->where('id', 35)->first();
        $now = now();
        ...
        if (DB::table('services')->where('id', 37)->exists()) {
            DB::table('services')->where('id', 37)->update($serviceData);
        } else {
        ...
        if (DB::table('providers')->where('id', 342)->exists()) {
            DB::table('providers')->where('id', 342)->update($providerData);
        } else {
        ...
        $existingApi = DB::table('apis')->where('id', 18)->first();
        $credentials = [
        ...
        if ($existingApi) {
            DB::table('apis')->where('id', 18)->update($apiData);
        } else {
    ...
    public function down(): void
    {
        ...
        DB::table('services')->where('id', 37)->delete();
        DB::table('providers')->where('id', 342)->delete();
        DB::table('apis')->where('id', 18)->delete();

        foreach (['commissions', 'apicommissions', 'apiproviders'] as $table) {
            if (Schema::hasTable($table)) {
                DB::table($table)->where('provider_id', 342)->delete();
            }
        }
    }
};
