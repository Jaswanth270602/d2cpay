...
use App\Http\Controllers\Agent\AurexaPayController;
use App\Models\Gatewayorder;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\DB;
use Throwable;

class AurexaPaySyncPendingPayins extends Command
{
            protected $signature = 'aurexapay:sync-pending
    ...
    protected $description = 'Sync pending AurexaPay payin orders from webhook logs / status API and credit wallet';
    ...
    public function handle(): int
    {
        ...
        $query = Gatewayorder::where('api_id', 19)->whereIn('status_id', [3, 9]);
        ...
        $orders = $query->get();
        if ($orders->isEmpty()) {
            $this->info('No pending AurexaPay payin orders found.');
            return self::SUCCESS;
        }

        $this->info('Processing ' . $orders->count() . ' pending order(s)...');

        $controller = new AurexaPayController();
        $synced = 0;
        $failed = 0;
        $errors = 0;
        ...
        foreach ($orders as $order) {
            ...
            try {
                ...
                if ($credited || ((int)$order->status_id === 1 && $before !== 1)) {
                    ...
                    $failed++;
                    $this->warn("Order {$order->id} marked failed at AurexaPay.");
                } else {
                    $this->line("Order {$order->id} still pending at AurexaPay.");
                }
            } catch (Throwable $e) {
                $errors++;
