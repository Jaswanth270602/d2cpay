...
<script>
    function createElectraOrder() {
        ...
        $.ajax({
            type: "POST",
            url: "{{ url('agent/add-money/v4/create-order') }}", // ✅ Payin 4 endpoint
            data: {
                _token: token,
                amount: amount
            },
            success: function (response) {
                $(".loader").hide();
                $("#proceedButton").prop("disabled", false); // re-enable
            ...
            error: function (xhr) {
                $(".loader").hide();
                $("#proceedButton").prop("disabled", false); // re-enable
                showErrorCard("Validation failed: " + (xhr.responseJSON?.message || "Unknown error"));
            }
        });
    }
    ...
    // Close button action → redirect back
    function closePayin() {
        window.history.back();
    }
</script>
    ...
    <div class="row">
        <div class="col-lg-6 col-md-12" style="margin-left:25%;margin-top:100px;">
            <div class="custom-card">
                <h6 class="card-title mb-1">Payin 4</h6>
                <p>Generate a QR instantly by entering the amount</p>
                <hr>
                <div class="mb-2">
                    <input type="text" class="form-control" id="amount" placeholder="Enter Amount">
                    <div id="errorMsg" class="error-text">⚠ Amount must be between 100 and 20000</div>
                </div>
                <div>
                    @csrf
                    ...
                    <button class="btn btn-secondary" type="button" onclick="closePayin()">
                        Close
                    </button>
                </div>
            </div>
