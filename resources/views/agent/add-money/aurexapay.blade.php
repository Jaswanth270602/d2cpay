...
<div class="main-content-body d-flex justify-content-center align-items-center" style="min-height:100vh;">
    <div class="col-md-5 unique-form-wrapper" id="form-wrapper">
        <div class="unique-card">
            <h3 class="unique-title">Payin 12</h3>
            ...
            <div class="form-group">
                ...
                <label class="form-label" for="amount">Enter Amount ({{ $min_amount ?? 500 }} to {{ $max_amount ?? 25000 }})</label>
...
<script>
    const MIN_AMOUNT = {{ $min_amount ?? 500 }};
    const MAX_AMOUNT = {{ $max_amount ?? 25000 }};
    ...
    function pollqpcOrderStatus() {
        ...
        $.ajax({
            type: 'POST',
            url: "{{ url('agent/add-money/v12/order-status') }}",
            data: {
    ...
    function createOrder(){
        ...
        $.ajax({
            type: "POST",
            url: "{{url('agent/add-money/v12/create-order')}}",
            data: {
                amount: amount,
                _token: token
            },
            success: function(msg){
                ...
                if (msg.status == 'success') {
                    ...
                    var payHref = msg.data.qrString || msg.data.upiLink || msg.data.paymentUrl || '#';
                    $("#qrStringBtn").attr('href', payHref);
                    if (!payHref || payHref === '#') {
                        $("#qrStringBtn").addClass('disabled').attr('aria-disabled', 'true');
                    } else {
                        $("#qrStringBtn").removeClass('disabled').removeAttr('aria-disabled');
                    }
